...
/**
 * Smart Pac-Man algorithm - uses pathfinding and ghost avoidance.
 */
public class SmartAlgo implements PacManAlgo {
    private int _count;
    ...
    public int move(PacmanGame game) {
        int dir = Game.ERR;
        int code = 0;
        boolean cyclic = ServerConfig.CYCLIC_MODE;
        ...
        if (ghostdis <= 3 && closestIndex >= 0 && ghosts[closestIndex].remainTimeAsEatable(code) > 1) {
    ...
    private static void printGhosts(GhostCL[] gs) {
        for (int i = 0; i < gs.length; i++) {
            GhostCL g = gs[i];
            System.out.println(i + ") status: " + g.getStatus() + ",  type: " + g.getType() +
                    ",  pos: " + g.getPos(0) + ",  time: " + g.remainTimeAsEatable(0));
        }
    }
